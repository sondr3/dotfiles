...
greatest of things, however with packages that come with Emacs you
need to do something extra for it to fetch the latest version. For
more info see [[https://github.com/jwiegley/use-package/issues/319][here]].  

First we need to add the Org mode ELPA package repository.
#+BEGIN_SRC emacs-lisp :tangle yes
