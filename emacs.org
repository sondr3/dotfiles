...
** Mode line
*** Spaceline
I honestly can't be arsed to configure the mode line a lot, I tried with
~smart-mode-line~ and it was a major PITA to get it working anywhere close to how
I wanted it to work. However, ~spaceline~ does exactly that. Neat.
...
| ~<SPC>f l~ | Change dictionary language      | ~my/cycle-languages~         |
| ~<SPC>f e~ | Go to next wrongly spelled word | ~flyspell-goto-next-error~   |
| ~<SPC>f w~ | Auto correct current word       | ~flyspell-auto-correct-word~ |
| ~<SPC> j~  | Jump to thing                   | ~avy-goto-char-timer~        |
| ~<SPC> l~  | Jump to line                    | ~avy-goto-line~              |
| ~<SPC>g s~ | Show git status                 | ~magit-status~               |
| ~<SPC>g c~ | Open a commit window            | ~magit-commit~               |
#+BEGIN_SRC emacs-lisp
  (use-package general
    :ensure t
    :config
    ...
    (general-define-key "w" 'save-buffer
                        "o" 'find-file
                        "b" 'switch-to-buffer
                        "fl" 'my/cycle-languages
                        "fe" 'flyspell-goto-next-error
                        "fw" 'flyspell-auto-correct-word
...
Should be self explanatory, I don't need to see 'em so I'll just hide 'em.
#+BEGIN_SRC emacs-lisp
  (setq org-hide-emphasis-markers t)
#+END_SRC
** Better bullets
Something I found on [[http://www.howardism.org/Technical/Emacs/orgmode-wordprocessor.html][Howardism.org]], this makes your bullet points in ~org-mode~
look nice. And I like nice.
#+BEGIN_SRC emacs-lisp
...
And then some for the headers. Not that They don't look nice already... they
just could look even better.
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  (let* ((variable-tuple (cond ((x-list-fonts "Source Code Pro") '(:font "Source Code Pro"))
    ...
    (custom-theme-set-faces 'user
                            ...
                            `(org-level-3 ((t (,@headline ,@variable-tuple :height 1.125))))
