...
After testing it out I realized I'd spend all my time trying to create various
hydras instead of using something like ~general.el~ to bind keys every which way I
want and then use ~which-key~ to display those.

** Key binds
And here the real fun begins.

#+BEGIN_SRC emacs-lisp
  (general-define-key "SPC" 'which-key-show-top-level)
#+END_SRC
* General
This is where non-specific stuff like spell checking and such goes.

** Auto completion
Although not something that I even used in Vim nor Sublime it's something that
I've had on my TODO-list for quite some time. And Emacs has a really nice auto
completion framework in ~Company~.
#+BEGIN_SRC emacs-lisp
  (use-package company
...
#+END_SRC
** ~anzu~
Although I never used it's counterpart in Vim it does look useful for both Emacs
and Vim.
#+BEGIN_SRC emacs-lisp
  (use-package anzu
    :delight anzu-mode " ⓐ"
...
* Languages
** Lisps
Since most of the languages in the Lisp family share very similar syntax I'll
group most of their settings in this group.
*** Lispy
~Lispy~ is a new mode for editing Lisps in the same vein as ~paredit~ and
~cleverparens~ but newer and better. Especially as it's written by ~abo-abo~ and
that guy is amazing.
#+BEGIN_SRC emacs-lisp
  (use-package lispy
    :delight lispy-mode " Ⓛ"
    ...
    (progn
      ...
      (defun my-lispy-on ()
        (when (member major-mode
                      '(emacs-lisp-mode scheme-mode lisp-interaction-mode))
          (lispy-mode)))

      (add-hook 'evil-insert-state-entry-hook #'my-lispy-on)
      (add-hook 'evil-insert-state-exit-hook #'my-lispy-off)))
#+END_SRC

*** Lispyville
Since I use Evil with Emacs I need Lispy to work well with it, and this is what
Lispyville does for me.
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
** Scheme
To edit and run and so on Scheme code we'll use ~Geiser~. It's pretty neat.
#+BEGIN_SRC emacs-lisp
  (use-package geiser
    :commands run-geiser)
#+END_SRC
