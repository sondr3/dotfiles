...
** Set some constants
We'll use our own ~post-init-hook~ to reset back some settings used by Emacs,
mostly to try and make the boot quicker, but afterwards we want the defaults.

#+BEGIN_SRC emacs-lisp
  (defconst amalthea--post-init-hook nil)
#+END_SRC

Then we'll override the build in file handler, and bind it to our own variable.
...
** Quick boot
First we disable the file name handler, which makes booting really, really
slow.
...
We don't want Emacs to be "constrained" by the default garbage collector settings.
...
However, we also want it back to the defaults at the end...
...
Next up on the menu is fixing the depth of the evaluation depth and recursion
limit.
...
#+END_SRC
** Journal
I used to write a journal, however it was written in DayOne because it seems as
a good idea at the time. Because of that it's completely locked behind their
app, and they upgraded it and that costs money. With org-journal it's completely
free and it's on org-mode, win, win win.
...
And then some configuration of org-mode:
